...
<html>
	...
	<body>
		<?php
			...
			if (isset($_SESSION['userid'])) 
			{
				...
				<div class = "main_container">
					<div class = "template_heading account_heading"><span class = "template_left">Country List</span> <span class="template_right"><a class="button round blue" href="<?php echo $_SERVER['HTTP_REFERER']; ?>">Back</a></span></div> 
					<div class = "main">
						<div class = "main_left cat_mail_left">
							<ul>
								<li class = "li_bold">sr. no</li>
								<li class = "li_bold">Country name</li>
								...
								<?php
									...
									while ($company_list =  mysql_fetch_assoc($rs))
									{
										...
										{	
											$cat_count_rs = mysql_query("select COUNT(country_id) from email where country_id = ".$cat_id);
										}
										else
										{
											$cat_count_rs = mysql_query("select COUNT(country_id) from email where country_id = ".$cat_id." and user_id = ".$user_id);
										}		
										
										$cat_count_list =  mysql_fetch_assoc($cat_count_rs);
										$total_count = $cat_count_list['COUNT(country_id)'];
										?><ul>
											<li><?php echo $index; ?></li>
											<li><?php echo $company_list['country_name']; ?></li>
										...
										<div class = "show_cat_imail show_cat_imail_<?php echo $cat_id; ?>" style = "display:none">
											<?php 
												if($_SESSION['type'] == 'admin')
												{	
													$cat_rs = mysql_query("select * from email where country_id = ".$cat_id);
												}
												else
												{
							...
							<div class = "main_right">
								<div>
									<form>
										...
										<div>
											<a id = "add_cat" class="button round blue" >Add Country</a>
										</div>
									</form>
								</div>
							</div>
		...
		<script>
				jQuery(document).ready(function(){
					jQuery('#add_cat').click(function(){
						...
						{	
							...
							var data = {'function':'SaveCountry','country_name':cat_name};
							jQuery.ajax({
				...
				function deleteCat(cat_id)
				{
					jQuery.prompt("Are you sure you want to delete this item? It cannot be restored at a later time!", {
						...
						submit: function(e,v,m,f){
							if(v)
							{
								var data = {'function':'DeleteCountry','id':cat_id};
								jQuery.ajax({
									url: 'controller/MasterController.php', 
									type: 'POST',
									data: data,
				...
				{
					location.href = "edit-country.php?id="+cat_id;
				} 
				var cat_id1 = 0;
