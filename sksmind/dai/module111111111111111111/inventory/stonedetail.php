...
<?php include($daiDir."head.php"); ?>	
	...
	<body class="no-skin">
				...
				<div class="main-content-inner">					
					<div class="page-content">
						...
						<?php
							...
							</div>							<button class="btn btn-info btn-update" onclick="updateData('');" style="float:right;margin-right:5%;" type="button">								<i class="ace-icon fa fa-check bigger-110"></i>								Save Detail							</button>
		...
		<script type="text/javascript">
			...
			function updateData(sku)			{				var data = jQuery('#update-form').serialize();								jQuery('#please-wait').show();								jQuery.ajax({					url: '<?php echo $daiUrl.'module/inventory/inventoryController.php'; ?>', 					type: 'POST',					data: data,							success: function(result)						{									var obj = jQuery.parseJSON(result);														alert(obj.message);							if(obj.status)							{								loadHistory(jQuery("input[name=sku]").val());							}						}				});			}
			function loadHistory(sku)
			{
				...
				jQuery.ajax({
					url: '<?php echo $daiUrl.'module/inventory/stoneData.php'; ?>', 
					type: 'POST',
					data: data,		
