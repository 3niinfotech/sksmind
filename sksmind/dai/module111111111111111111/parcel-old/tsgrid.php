	...
	<table  id="dynamic-table" class="inventory-table table table-striped table-bordered table-hover">
		...
		<tbody>
			...
			<tr class="<?php echo $class; ?>">
				...
				if(strtoupper($value['group_type']) =='PARCEL' && $value['parcel_products'] !="" )
				{	
					 echo '<a style="cursor:pointer;" onClick="loadBox('.$id.',0)" >  <i class="ace-icon fa  fa-folder bigger-120"></i> '.$value['mfg_code'];echo($value['diamond_no']!="" )?'-'.$value['diamond_no']:''.'</a>'; 
				}				 
				else
				{
					echo $value['mfg_code'];echo($value['diamond_no']!="" )?'-'.$value['diamond_no']:'';
				}
				?>
