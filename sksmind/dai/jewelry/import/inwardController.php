...
{
	$rs = mysqli_query($cn,"TRUNCATE TABLE jew_temp");
	if(!$rs)
	{
		$rs = mysqli_error();
		$_SESSION['error'] = $rs;	
	...
	{
		$rs = mysqli_query($cn,"TRUNCATE TABLE jew_temp_loose");
		if(!$rs)
		{
			$rs = mysqli_error();
			$_SESSION['error'] = $rs;	
...
function looseSave($cn)
{
	...
	else
	{
		...
		{	
			...
			if(isset($_POST["id"]))
			{
				$rs = $model->looseupdateData($_POST);
			}
			else
			{
				$rs = $model->looseSaveData($_POST);
			}		
...
function colletSave($cn)
{
	if(isset($_POST["import"]))
	{
		if ( isset($_FILES["filename"]) && isset($_FILES["filename"]["name"]) && $_FILES["filename"]["name"]!=""  ) 
		{
			...
			else 
			{
				...
				$rs = $model->colletSave($path.$fname);
	...
	else
	{
		...
		{	
			...
			if(isset($_POST["id"]))
			{
				$rs = $model->colletupdateData($_POST);
			}
			else
			{
				$rs = $model->colletSaveData($_POST);
			}		
