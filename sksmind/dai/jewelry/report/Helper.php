...
class Helper
{
	...
	function getInsertString($post)
	{
		...
		foreach($post as $key=>$value)
		{
			...
			$nameString .= $key;			
			$valueString .= "'".mysqli_real_escape_string($value)."'";
	...
	function getUpdateString($post)
	{
		...
		foreach($post as $key=>$value)
		{
			...
			$valueString .= $key."='".mysqli_real_escape_string($value)."'";			
			...
			{	
				foreach(explode(',',$row['side_stone']) as $k=>$pid )
				{
					...
					while($row1 = mysqli_fetch_assoc($rs1))
					{
						...
						if($row1['outward_parent'] != 0)
						{
							$parentData = $this->getSideProductDetail($row1['outward_parent']);	
							$row1['sku'] = $parentData['sku'];
						}
						$row['record'][$pid] =  $row1;			
					}
				}
	...
	public function getReportMemo($post)
    {
		...
		{	
			...
			if($report == "sale" || $report =="export" || $report == "close_sale" )	
				$inward = " and type='sale'";
			
			if($report == "memo" || $report == "close_memo" || $report =="consign" )	
				$inward = " and type='consign'";
			
			if($report == "sale")
			$status = " and status <> 'close_sale'";
			
			if($report == "memo")
			$status = " and status <> 'close_consign' ";
