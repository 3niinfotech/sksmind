		...
		function mainCheck(temp)
			{
				...
				if($(temp).is(':checked'))
				{
					...
					$(temp).parents('.divTableRow ').addClass('infobox-blue infobox-dark');
				...
				else
				{
					...
					$(temp).parents('.divTableRow ').removeClass('infobox-blue infobox-dark');
			...
			function loadGrid(data)
			{
					...
					jQuery.ajax({
						url: '<?php echo $jewelryUrl.'inventory/colletInGrid.php?form_type=';?>'+ftype, 
						type: 'POST',
						data:data,	
			...
			function removeCollet()
			{
				
				if(confirm('Are You sure to Remove Stone ?') )
				{
					...
					var data = {'ids':checkValues,'fn':'removeCollet'};	
			...
			function toRepair()
			{
				...
				if(confirm('Are You sure to Repair Stone ?') )
				{
					 var checkValues = $('.invenotry-cgrid input[name=mainstone]:checked').map(function()
					{
						return $(this).val();
					...
					{	
						...
						jQuery.ajax({
							url: '<?php echo $daiUrl.'jewelry/inventory/colletRepair.php' ?>', 
							type: 'POST',
							data: data,		
							success: function(result)
								{		
