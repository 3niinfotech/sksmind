...
<div class="divTable" >
	...
	<div class="divTableRow infobox-grey infobox-dark">		
		...
		<?php else:  */ ?>
				...
				<div class="divTableCell a-right  carats" id="carat-<?php echo $value['id'].$i ?>">  <?php echo (isset($value['carat']))?$value['carat']:'&nbsp;'?>  </div>
				
				<div class="divTableCell a-right price" style="padding:0px"> <input style="width: 100%; height: 100%; border: 0px none; text-align: right;" type="text" id="price-<?php echo $value['id'].$i ?>" value="<?php echo $sprice?>" onBlur="changeTextPrice(this.value,<?php echo $value['id'] ?>,<?php echo $value['id'].$i ?>)"></div>
				<div class="divTableCell a-right amount1" id="amount-<?php echo $value['id'].$i ?>">  <?php echo ($value['sell_amount'] ==0)?$value['amount']:$value['sell_amount'] ?></div>
...
function changeTextPrice(price,id,idi)
{
	var carat = parseFloat($('#carat-'+idi).html());
	var amount = parseFloat(price) * carat;
	$('#amount-'+idi).html(amount.toFixed(2));
	...
	if(!isNaN(amount))
	{
		...
		jQuery.ajax({
		url: '<?php echo $daiUrl.'jewelry/import/inwardController.php'?>', 
		type: 'POST',
		data: data,		
		success: function(result)
		{		
