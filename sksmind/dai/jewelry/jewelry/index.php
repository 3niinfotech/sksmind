		...
		<div class="main-container ace-save-state" >
			...
			<div class="main-content" id="jmain-container">
						...
						<?php
							...
							?>
							<form id="jew-form" action="<?php echo $mainUrl."pdf/file/jewelry.php"?>" method="post" target="_blank">
							<input type="hidden" value="" name="ids" id="exportSku">
							<div class="form-group col-sm-12">
								<div style="float:left;margin-top: 10px;"><label class="pos-rel">
									<input class="ace" onclick="allCheck(this)" type="checkbox">
									<span class="lbl">&nbsp;&nbsp; Select All</span>
								</label>
								</div>
								<button class="btn btn-info" type="button" onclick="ExpPrintFull()" style="float: right; margin: 0px;margin-right:10px;">
									<i class="ace-icon fa fa-print bigger-130"></i>
									Full Print
								</button>
								<button class="btn btn-info" type="button" onclick="ExpPrint()" style="float: right; margin: 0px;margin-right:10px;">
									<i class="ace-icon fa fa-print bigger-130"></i>
									Print
								</button>
							</div>
...
<script src="<?php echo  $daiUrl;?>assets/js/bootbox.js"></script>
		...
		<script type="text/javascript">
			...
			function loadGrid(data)
			{
					...
					jQuery.ajax({
						url: '<?php echo $jewelryUrl.'jewelry/jgrid.php' ?>', 
						type: 'POST',
						data:data,	
			...
			function loadMemoForm(rs,type)
				{
					...
					{	
						...
						jQuery.ajax({
							url: '<?php echo $jewelryUrl.'jewelry/sendToOutward.php' ?>', 
							type: 'POST',
							data: data,		
							success: function(result)
								{		
				...
				function tojobRepair(id)
				{
					...
					jQuery.ajax({
						url: '<?php echo $jewelryUrl.'jewelry/jobWork.php' ?>', 
						type: 'POST',
						data: data,		
						success: function(result)
							{		
				...
				function ExpPrint()
				{
					...
					if(checkValues.length)
					{
						$('#jew-form').attr('action', "<?php echo $mainUrl."pdf/file/jewelry.php"?>");
						$('#exportSku').val(checkValues);
						$('#jew-form').submit();
					}
					else
					{
				...
				function ExpPrintFull()
				{
					...
					if(checkValues.length)
					{
						$('#jew-form').attr('action', "<?php echo $mainUrl."pdf/file/jewelryfull.php"?>");
						$('#exportSku').val(checkValues);
						$('#jew-form').submit();
					}
					else
				...
				function allCheck(temp)
				{
					...
					if($(temp).is(':checked'))
					{
						$('.cgrid-header.green input:checkbox').prop('checked',true);
					}
					else
					{
						$('.cgrid-header.green input:checkbox').prop('checked',false);
					}			
				}
		</script>
...
.jewelry-col1{width:15%;}
.jewelry-col2,.jewelry-col4{width:7%;}
.jewelry-col3{width:78%;}
